...
5. In Texas, `fetch_tasb_policy` with the district's TASB key reads FO(LOCAL), which is the board policy the handbook refers to, with its update number and issue date. FO(LEGAL) is the statute and is the same for every district, so it never establishes a district's own policy, and it is omitted unless you pass `include_legal`. Leave it omitted: it is about 35,000 characters a district and it is the reason a scan runs out of context before it runs out of districts. If TASB answers with its member picker, that district is not publishing a manual there at the moment; go to its own site rather than recording it as unknown.
