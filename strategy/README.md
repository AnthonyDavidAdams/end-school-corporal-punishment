# Strategy: how corporal punishment ends in US schools

Corporal punishment in US schools will not end with one federal law. It ends the way it has been ending for fifty years: district by district until a state's legislature is ratifying a fact rather than making a decision, then state by state until Congress is ratifying a fact. This directory holds the theory of change, the playbooks for each lever, and the case studies of every win we can document.

## The theory of change

1. **Make the practice visible at the district level.** Most parents in a paddling district do not know their district paddles, or how often, or whom. The federal Civil Rights Data Collection counts it every two years by school. Joining those counts to district policies and publishing them by name is the foundation everything else stands on. See `data/` and `tasks/README.md`.
...
- A single sourced dataset of district policies, joined to federal counts, updated by agents instead of a grad student every five years.
- A verified claims registry so every advocate, journalist, and legislator quotes the same current number.
- Playbooks written from what actually worked, with sources, rather than general advocacy advice.
- A free replacement curriculum so "what instead" has an answer any principal can adopt on a Monday.
