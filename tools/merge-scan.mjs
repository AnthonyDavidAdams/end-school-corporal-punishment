...
for (const f of process.argv.slice(2)) {
  for (const r of JSON.parse(readFileSync(f, "utf8"))) {
    ...
    const entry = r.status === null ? Object.fromEntries(Object.entries({ phone_policy: r.phone_policy, ai_policy: r.ai_policy, start_times: r.start_times, archived_url: r.archived_url, document_text_path: r.document_text_path }).filter(([, v]) => v)) : { name: r.name, county: r.county || null, nces_id: r.nces_id || null, status: r.status, source: r.source || null, quote: r.quote || null, policy_code: r.policy_code || null, crdc_students_latest: r.students_2023_24 ?? null, last_verified: r.last_verified || new Date().toISOString().slice(0, 10), ...(r.method ? { method: r.method } : {}), ...(r.parent_control ? { parent_control: r.parent_control } : {}), ...(r.quotes?.length ? { quotes: r.quotes } : {}), ...(r.documents?.length ? { documents: r.documents } : {}), ...(r.phone_policy ? { phone_policy: r.phone_policy } : {}), ...(r.ai_policy ? { ai_policy: r.ai_policy } : {}), ...(r.start_times ? { start_times: r.start_times } : {}), ...(r.archived_url ? { archived_url: r.archived_url } : {}), ...(r.document_text_path ? { document_text_path: r.document_text_path } : {}), ...(contact ? { contact } : {}), notes: [r.opt_out_or_consent, r.notes, r.status === "unknown" && r.searched ? `Searched: ${[].concat(r.searched).join("; ")}` : null].filter(Boolean).join(" ") || null };
