...
for (const f of readdirSync(join(root, "data/districts")).filter(f => f.endsWith(".yaml"))) {
  ...
  for (const d of doc.districts) {
    // Nothing is sent about a district nobody has opened the policy for, and nothing is sent to a
    // district whose own record says it already prohibits it -- there is no notice to give.
    if (!d.source || !d.quote || !STATUS_LINE[d.status]) continue;
    if (d.status === "bans") continue;
